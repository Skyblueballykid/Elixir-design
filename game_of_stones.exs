  ...
  defp start_game! do
    ...
    receive do
      {player, current_stones} ->
        IO.puts("Starting the game! It's player #{player}'s turn.
  ...
  defp take() do
    ...
    receive do
      ...
      {:error, reason} ->
        ...
        take()
    after
      5000 -> IO.puts(:standard_error, "Server timeout.")
    end
  end
end
...
defmodule GameServer do
  ...
  defp do_take({sender, player, num_stones_take, current_stones_count})
       when not is_integer(num_stones_take) or
    ...
    send(
      sender,
      {:error,
       "You can only take from 1 to 3 stones, and you can't exceed the count of the entire pile, asshole."}
    )
